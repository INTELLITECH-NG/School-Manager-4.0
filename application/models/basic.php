...
{			
	...
	function update_last_roll($session_id,$dept_id,$shift_id){
		$q="insert into class_roll (session_id,dept_id,shift_id,last_used_roll) select $session_id,$dept_id,$shift_id,start_roll from department where id=$dept_id ON DUPLICATE KEY UPDATE last_used_roll=last_used_roll+1";
		return $result=$this->execute_complex_query($q);
	}
